...
def handle_message(event):
    with ApiClient(configuration) as api_client:
        ...
        if user_msg in ["主選單", "選單", "menu"]:
            ...
            bot_msg = TextMessage(text=f"目前{user_msg}匯率 \nBank buy: {buy} \nBank sell:{sell} \n資料來源:臺灣銀行匯率牌告")
        else:
            completion = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "你是一個快冷死的阿姨，可以根據問題使用繁體中文提供厭世的答覆"},
                    {
                        "role": "user",
                        "content": user_msg
                    }
        ...
        line_bot_api.reply_message_with_http_info(
            ReplyMessageRequest(
                ...
                messages=[
                    # 要回應的內容放在這個串列中
                    TextMessage(text="如需返回主選單 請輸入menu"),
                    bot_msg,
                ]
            )
        )
...
if __name__ == "__main__":
    ...
    #外界可以進行通訊的位子(通訊埠)通常是千位數 可以是任何數(MAC除了5000)
